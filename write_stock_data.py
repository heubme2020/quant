...
def write_symbol_data_into_table(exchange, stock_list):
    ...
    stock_table = Table(exchange_stock_symbol_name, metadata,  autoload_with=engine)
    conn = engine.connect()
    company_list = []
...
def write_exchange_stock_symbol_data(exchange):
    ...
    if exchange_stock_symbol_name in table_names:
        ...
        #建立链接
        stock_symbol_table = Table(exchange_stock_symbol_name, metadata, autoload_with=engine)
        conn = engine.connect()
...
def write_exchange_income_data(exchange):
    ...
    if income_exchange_name in table_names:
        ...
        for i in range(len(stock_symbol_data)):
            ...
            last_date = conn.execute(text(f"SELECT MAX(endDate) FROM {income_exchange_name} WHERE symbol = '{symbol}'"))
    ...
    if balance_exchange_name in table_names:
        ...
        for i in range(len(stock_symbol_data)):
            ...
            if ipo_date == '':
                ipo_date = '19000101'
            ipo_date = "-".join([ipo_date[:4], ipo_date[4:6], ipo_date[6:]])
            last_date = conn.execute(text(f"SELECT MAX(endDate) FROM {balance_exchange_name} WHERE symbol = '{symbol}'"))
            last_date = last_date.scalar()
            if last_date is None:
                last_date = ipo_date
            else:
    ...
    if cashflow_exchange_name in table_names:
        ...
        for i in range(len(stock_symbol_data)):
            ...
            if ipo_date == '':
                ipo_date = '19000101'
            ipo_date = "-".join([ipo_date[:4], ipo_date[4:6], ipo_date[6:]])
            last_date = conn.execute(text(f"SELECT MAX(endDate) FROM {cashflow_exchange_name} WHERE symbol = '{symbol}'"))
            last_date = last_date.scalar()
            if last_date is None:
                last_date = ipo_date
            else:
...
def get_quarter_end_date(date_str):
    ...
    # 计算下个季度的第一天
    current_quarter = (date_object.month - 1) // 3 + 1
    next_quarter_month = current_quarter * 3 + 1
    next_quarter_year = date_object.year + 1 if next_quarter_month > 12 else date_object.year
    first_day_of_next_quarter = datetime.datetime(next_quarter_year, next_quarter_month if next_quarter_month <= 12 else 1, 1)

    # 计算本季度的最后一天（下个季度第一天的前一天）
    last_day_of_quarter = first_day_of_next_quarter - datetime.timedelta(days=1)

    return last_day_of_quarter.strftime('%Y%m%d')
...
def init_exchange_dividend_data(exchange):
    ...
    for i in range(len(stock_symbol_data)):
        ...
        enterprise_values_data['endDate'] = enterprise_values_data['endDate'].apply(get_quarter_end_date)
...
def write_exchange_dividend_data(exchange):
    ...
    if dividend_exchange_name in table_names:
        ...
        #建立连接
        metadata = MetaData()
        dividend_table = Table(dividend_exchange_name, metadata, autoload_with=engine)
        conn = engine.connect()
        #开始遍历股票添加数据
        for i in range(len(stock_symbol_data)):
            print(dividend_exchange_name + ':' + str(float(i) / len(stock_symbol_data)))
            ...
            if ipo_date == '':
                ipo_date = '19000101'
            ipo_date = "-".join([ipo_date[:4], ipo_date[4:6], ipo_date[6:]])
            last_date = conn.execute(text(f"SELECT MAX(endDate) FROM {dividend_exchange_name} WHERE symbol = '{symbol}'"))
            last_date = last_date.scalar()
            if last_date is None:
                last_date = ipo_date
            else:
...
def write_daily_data_into_table(daily_exchange_name, stock_symbol_data):
    ...
    for i in range(len(stock_symbol_data)):
        ...
        if ipo_date == '':
            ipo_date = '19000101'
        ipo_date = "-".join([ipo_date[:4], ipo_date[4:6], ipo_date[6:]])
        last_date = conn.execute(text(f"SELECT MAX(date) FROM {daily_exchange_name} WHERE symbol = '{symbol}'"))
        last_date = last_date.scalar()
        if last_date is None:
            last_date = ipo_date
        else:
...
def write_exchange_financial_data(exchange):
    ...
    t4 = threading.Thread(target=write_exchange_dividend_data, args=(exchange,))
    t1.start()
    t2.start()
    t3.start()
    t4.start()
...
def write_exchange_data(exchange):
    ...
    write_exchange_indicator_data(exchange)
    write_exchange_daily_data(exchange)
...
def delete_exchange_daily_data(exchange):
    exchange = exchange.lower()
    inspector = sa.inspect(engine)
    table_names = inspector.get_table_names()
    drop_list = []
...
def refresh_exchange_financial_data(exchange):
    delete_exchange_financial_data(exchange)
    write_exchange_financial_data(exchange)


def refresh_exchange_daily_data(exchange):
    delete_exchange_daily_data(exchange)
    write_exchange_daily_data(exchange)


if __name__ == "__main__":
    exchange_list = ['Shanghai', 'Shenzhen']
    for exchange in exchange_list:
        refresh_exchange_financial_data(exchange)
        refresh_exchange_daily_data(exchange)